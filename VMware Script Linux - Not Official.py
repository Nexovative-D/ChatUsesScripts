    ...
    if platform.system() == "Windows":
        ...
        try:
            if _shutil.which("zenity"):
                result = subprocess.run(["zenity", "--question", "--title", title, "--text", msg],
                                         capture_output=True, timeout=120)
                return result.returncode == 0
            elif _shutil.which("kdialog"):
                result = subprocess.run(["kdialog", "--title", title, "--yesno", msg],
                                         capture_output=True, timeout=120)
                return result.returncode == 0
        except Exception:
            return True
        try:
            answer = input(f"[{title}] {msg}\n(y/n): ").strip().lower()
...
def _write_autostart_flags_json(folder):
    ...
    if one was running (extra video IDs / VNC config already persist in their own
...
def _generate_relaunch_script_linux(folder, base_name, flags):
    """Linux equivalent of _generate_relaunch_batch()/_generate_relaunch_script_macos() --
    same steps (stop the running bot, refresh version.json for the record, launch the new
    instance with everything auto-starting). The actual shell script content is identical
...
def trigger_relaunch_pipeline(reason, version_data=None, source_path=None):
    ...
    source_path: which file's content to actually carry over to the new autostart
    copy, for the local file-edit-trigger case. Defaults to the currently-running
    ...
    if version_data is not None:
        ...
        try:
            shutil.copyfile(source_path, os.path.join(folder, autostart_filename))
        except Exception as e:
    ...
    if platform.system() == "Linux":
        script_path = _generate_relaunch_script_linux(folder, base_name, flags)
        if not script_path:
            _autoupdate_relaunch_triggered = False
            return
        # Linux has no single universal terminal the way macOS has Terminal.app --
        # try the common ones in rough order of how likely they are to be installed,
        # falling back to a detached background launch (output redirected to a log
        # file so it's still checkable) if none of them exist at all.
        launched = False
        terminal_attempts = [
            ...
            ["x-terminal-emulator", "-e", "bash", "-c", f'"{script_path}"; exec bash'],
            ["xterm", "-e", "bash", "-c", f'"{script_path}"; exec bash'],
        ]
        import shutil as _shutil
        for attempt in terminal_attempts:
            if _shutil.which(attempt[0]):
                try:
                    subprocess.Popen(attempt, close_fds=True)
                    print(f"[AutoUpdate] Launched {os.path.basename(script_path)} via {attempt[0]}. Exiting so it can take over...")
                    launched = True
                    break
                except Exception:
                    continue
        if not launched:
            try:
                log_path = os.path.join(folder, "run_update.log")
                with open(log_path, "wb") as logf:
                    subprocess.Popen(["/bin/bash", script_path], stdout=logf, stderr=subprocess.STDOUT,
                                      start_new_session=True, close_fds=True)
                print(f"[AutoUpdate] No terminal emulator found -- launched {os.path.basename(script_path)} "
...
_update_splash(35, "Importing VNC control API...")
...
# ── Optional deps for Music/Video/Soundboard/Flask dashboard (transferred from chatuses.py) ──
try:
    import vlc as _vlc
    vlc_available = True
except ImportError:
...
def console_log(level, msg):
    ...
    (adapted from chatuses.py's console_log). Feeds both stdout and the GUI console."""
...
_update_splash(80, "Initializing VMware vmrun manager...")
...
def start_tray_icon():
    ...
    if platform.system() in ("Darwin", "Linux"):
        ...
        # on Windows) creates a run-loop ownership conflict that can corrupt Tcl's
        # notifier and crash with "Tcl_WaitForEvent: Notifier not initialized" --
        # possibly not immediately, and not necessarily from code that looks related.
        # Linux: pystray's GTK/AppIndicator backend is generally more forgiving about
        # threading than macOS's Cocoa, but run_detached() is pystray's own documented
        # answer for ANY app with an existing main loop (Tkinter's mainloop() here),
        # not a macOS-specific fix -- applying it here too rather than assuming the
        # threaded approach is safe just because it's less likely to crash outright.
        try:
            _tray_icon.run_detached()
            _tray_thread = None
            print(f"[Tray] System tray icon started ({platform.system()} run_detached).")
            return
        except Exception as e:
...
# ========================= VNC KEY MAP (VMware-styled keyboard input) =========================
# vmrun has no keystroke-injection command, so keyboard/mouse control goes
# over VNC (vncdotool) directly to the VM console, same as the VMware-styled bot.
# Direct X11 keysym codes are used to bypass bugs in the vncdotool library.
SCANCODES = {
...
# Letters and digits map to themselves — vncdotool accepts the literal character.
for _c in "abcdefghijklmnopqrstuvwxyz0123456789":
    SCANCODES[_c] = _c
del _c

# ── Shared, thread-safe VNC connection ──
_vnc_client = None
_vnc_lock   = threading.Lock()
_vnc_cursor = {"x": 512, "y": 384}

def _vnc_clear_stuck_modifiers(client):
    """Releases every modifier key, in case a previous keyDown never got its keyUp."""
    for name in ("shift", "ctrl", "alt", "win", "capslock"):
        mapped = SCANCODES.get(name)
        if mapped:
            try:
                client.keyUp(mapped)
            except Exception:
...
def vnc_connect_fresh():
    """Disconnects any existing session and opens a brand-new VNC connection."""
    global _vnc_client
    if _vnc_client:
        try:
            _vnc_client.disconnect()
        except Exception:
            pass
        _vnc_client = None
    try:
        _vnc_client = vnc.connect(f"{VNC_HOST}::{VNC_PORT}", password=str(VNC_PASSWORD))
        _vnc_clear_stuck_modifiers(_vnc_client)
        return _vnc_client
    except Exception as e:
        print(f'[VNC] Connect failed: Python exception: "{traceback.format_exc()}"')
        _vnc_client = None
        return None

def vnc_disconnect_now():
    """Closes the current VNC session right away. Called after every key/combo/mouse
    action finishes sending, so the connection is only ever open for the split-second
    it takes to deliver that one input -- not held open between chat commands."""
    global _vnc_client
    if _vnc_client:
        try:
            _vnc_client.disconnect()
...
def vnc_key_down(key_name):
    ...
    with _vnc_lock:
        ...
        if not client:
            return
        try:
            client.keyDown(mapped)
        except Exception as e:
            print(f'[VNC] keyDown error: Python exception: "{traceback.format_exc()}"')
        finally:
            vnc_disconnect_now()
...
def vnc_key_up(key_name):
    ...
    with _vnc_lock:
        ...
        try:
            ...
            print(f'[VNC] keyUp error: Python exception: "{traceback.format_exc()}"')
        finally:
            vnc_disconnect_now()
...
def send_combo(keys):
    ...
    with _vnc_lock:
        ...
        mapped_keys = [SCANCODES.get(k, k) for k in key_names]
        try:
            ...
            for k in reversed(mapped_keys):
                ...
                time.sleep(0.01)
            vnc_disconnect_now()
    print(f"[KB] Combo sent: {'+'.join(key_names)}")
...
# on Windows/Linux, 'fusion' (VMware Fusion) on macOS. Every vmrun call in this file uses
# this instead of a hardcoded 'ws', since Fusion silently rejects/misbehaves with the
# wrong target type.

def get_vmrun_path():
    """Locates vmrun.exe, the VMware Workstation command-line control tool."""
    possible_paths = [
...
# ── VM registry (vms.json) ──
# vmrun has no "list all registered VMs" command (only running ones), so VMs
# are registered locally by alias -> .vmx path, the VMware-styled way.
VM_REGISTRY_FILE = "vms.json"

def _load_vm_registry():
    try:
        if os.path.exists(VM_REGISTRY_FILE):
            with open(VM_REGISTRY_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception as e:
        print(f'[VM Registry] Load error: Python exception: "{traceback.format_exc()}"')
    return {}

def _save_vm_registry(registry):
    try:
        with open(VM_REGISTRY_FILE, "w", encoding="utf-8") as f:
            json.dump(registry, f, indent=2, ensure_ascii=False)
    except Exception as e:
        print(f'[VM Registry] Save error: Python exception: "{traceback.format_exc()}"')

def get_vm_list():
    """Returns the .vmx paths of every VM registered in vms.json."""
    registry = _load_vm_registry()
    return list(registry.values())

def vmrun_is_running(vmx_path: str) -> bool:
    """Checks 'vmrun list' to see if a given .vmx is currently powered on."""
    if not vmx_path or not VMRUN_PATH or not os.path.exists(VMRUN_PATH):
        return False
    try:
        result = subprocess.run([VMRUN_PATH, "-T", VMRUN_TARGET_TYPE, "list"],
...
def get_vboxmanage_path():
    """Locates VBoxManage.exe, VirtualBox's command-line control tool -- this script is
    VMware-first, but OS Voting rows (and the single-VM config) can ALSO point at a VBox
    VM, so this needs to be findable here too, not just in the VBox-flavored script."""
    possible_paths = [
...
current_vm_backend = "vmware"  # "vmware" or "vbox" -- which backend controls the CURRENTLY
                                # ACTIVE vm (VMX_PATH when vmware, VM_NAME-style identifier
                                # when vbox). Every VM-lifecycle command and OS Voting row
                                # checks this to know which tool to use.

def get_all_vbox_vms(vbox_path=None):
    """Returns every VM registered with VirtualBox (VBoxManage lists these natively --
    unlike vmrun, no local alias->path registry file is needed)."""
...
def get_vbox_snapshots(vm_name, vbox_path=None):
    """Lists snapshot names for a VBox VM via 'VBoxManage snapshot <vm> list'."""
    vbox_path = vbox_path or VBOXMANAGE_PATH
    snaps = []
    try:
...
def vbox_is_running(vm_name, vbox_path=None):
    vbox_path = vbox_path or VBOXMANAGE_PATH
    if not vm_name or not vbox_path:
        return False
    try:
        res = subprocess.run([vbox_path, "list", "runningvms"], capture_output=True, text=True, timeout=15)
        return f'"{vm_name}"' in (res.stdout or "")
    except Exception as e:
        print(f'[VBoxManage] list error: Python exception: "{traceback.format_exc()}"')
        return False

def vbox_start(vm_name, gui=True, vbox_path=None):
    vbox_path = vbox_path or VBOXMANAGE_PATH
    mode = "gui" if gui else "headless"
    return subprocess.run([vbox_path, "startvm", vm_name, "--type", mode], capture_output=True, text=True)

def vbox_stop(vm_name, hard=True, vbox_path=None):
    vbox_path = vbox_path or VBOXMANAGE_PATH
    verb = "poweroff" if hard else "acpipowerbutton"
    return subprocess.run([vbox_path, "controlvm", vm_name, verb], capture_output=True, text=True)

def vbox_reset(vm_name, vbox_path=None):
    vbox_path = vbox_path or VBOXMANAGE_PATH
    return subprocess.run([vbox_path, "controlvm", vm_name, "reset"], capture_output=True, text=True)

def vbox_revert_to_snapshot(vm_name, snapshot_name, vbox_path=None):
    vbox_path = vbox_path or VBOXMANAGE_PATH
    return subprocess.run([vbox_path, "snapshot", vm_name, "restore", snapshot_name], capture_output=True, text=True)

def vbox_take_snapshot(vm_name, snapshot_name, vbox_path=None):
    vbox_path = vbox_path or VBOXMANAGE_PATH
    return subprocess.run([vbox_path, "snapshot", vm_name, "take", snapshot_name, "--live"], capture_output=True, text=True)

def vbox_toggle_internet(vm_name, enable, vbox_path=None):
    """Live, no-restart-needed internet toggle for a VBox VM -- the VBoxManage equivalent
    of this script's own writeVariable-based vmrun toggle further down."""
    vbox_path = vbox_path or VBOXMANAGE_PATH
    state = "on" if enable else "off"
    try:
        res = subprocess.run([vbox_path, "controlvm", vm_name, "setlinkstate1", state], capture_output=True, text=True, timeout=30)
    except Exception as e:
        return subprocess.CompletedProcess([], 1, "", str(e))
    return res

def vbox_type_text(vm_name, text, vbox_path=None):
    """Types text into a VBox VM via VBoxManage's own keyboardputstring -- no COM session
    needed, matching how this script already drives VMware VMs by shelling out to vmrun."""
    vbox_path = vbox_path or VBOXMANAGE_PATH
...
def get_backend_for_vm(vm_identifier):
    ...
    OS Voting rows (e.g. the single-VM VMX_PATH in non-voting mode)."""
    for e in OS_LIST:
        if e.get("vm") == vm_identifier:
            return e.get("backend", "vmware")
...
def _run_scheduled_action(action: str, label: str):
    ...
    if action == "revert":
        def _do_revert():
            ...
            try:
                ok, _ = retry_vbox(
                    lambda: _checked(vm_stop(VMX_PATH, current_vm_backend, hard=True)),
                    attempts=3, delay=3, source="Scheduler/poweroff"
                )
                time.sleep(3)
                ok2, _ = retry_vbox(
                    lambda: _checked(vm_revert_to_snapshot(VMX_PATH, SNAPSHOT_NAME, current_vm_backend)),
                    attempts=3, delay=3, source="Scheduler/snapshot"
                )
                time.sleep(3)
                ok3, _ = retry_vbox(
                    lambda: _checked(vm_start(VMX_PATH, current_vm_backend, gui=True)),
        ...
        def _do_restart():
            ...
            try:
                ok, _ = retry_vbox(
                    lambda: _checked(vm_reset(VMX_PATH, current_vm_backend)),
...
_update_splash(85, "Connecting to VMware VNC console...")
# Keyboard/mouse input is injected over VNC (VMware Workstation exposes a VNC
# server per-VM); vmrun itself has no keystroke/mouse injection capability.
VNC_CONFIG_FILE = "vmware_vnc_config.json"
VNC_CONFIG = {"host": "localhost", "port": 5900, "password": "1234"}

def load_vnc_config():
    global VNC_CONFIG, VNC_HOST, VNC_PORT, VNC_PASSWORD
    try:
        if os.path.exists(VNC_CONFIG_FILE):
            with open(VNC_CONFIG_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            VNC_CONFIG.update(data)
    except Exception:
        pass
    VNC_HOST     = VNC_CONFIG.get("host", "localhost")
    VNC_PORT     = int(VNC_CONFIG.get("port", 5900))
    VNC_PASSWORD = VNC_CONFIG.get("password", "1234")

def save_vnc_config():
    safe_json_dump(VNC_CONFIG_FILE, VNC_CONFIG)
...
def retry_vbox(fn, attempts=3, delay=3, source="VBox"):
    ...
    fn must be a zero-argument callable wrapping a vmrun/subprocess call.
    """
    last_exc = None
...
def run_single_bot_command(line):
    ...
    try:
        if cmd in ("type", "text", "say"):
            ...
            try:
                _checked(vm_stop(VMX_PATH, current_vm_backend, hard=True))
                time.sleep(3)
                _checked(vm_revert_to_snapshot(VMX_PATH, SNAPSHOT_NAME, current_vm_backend))
                time.sleep(3)
...
current_os_vm = None     # currently running OS's VM name (used as active VMX_PATH target)
...
def switch_os(target_entry, announce=True):
    ...
    """
    global current_os_vm, VMX_PATH, os_switch_in_progress, os_votes, os_vote_start_time, current_vm_backend
    ...
    try:
        ...
        target_backend = target_entry.get("backend", "vmware")
        if not target_vm:
    ...
    def _speak():
        try:
            ...
            if platform.system() == "Darwin":
                ...
                # No single standard TTS command on Linux -- tries espeak-ng, then espeak,
                # then spd-say (speech-dispatcher), whichever is actually installed, since
                # availability genuinely varies by distro. All three take a words-per-minute
                # style rate; volume (0-100) maps reasonably well to espeak's -a (amplitude,
                # 0-200 scale) and spd-say's -i (0-100 scale already).
                import shutil as _shutil
                wpm = max(80, min(450, rate))
                volume = int(SOUND_CONFIG.get("tts_volume", 100))
                if _shutil.which("espeak-ng"):
                    subprocess.run(["espeak-ng", "-s", str(wpm), "-a", str(min(200, volume * 2)), text],
                                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30)
                elif _shutil.which("espeak"):
                    subprocess.run(["espeak", "-s", str(wpm), "-a", str(min(200, volume * 2)), text],
                                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30)
                elif _shutil.which("spd-say"):
                    subprocess.run(["spd-say", "-r", str(max(-100, min(100, int((rate - 175) / 2.75)))),
                                     "-i", str(max(0, min(100, volume))), text],
                                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30)
                else:
                    print("[Speech] No TTS engine found -- install espeak-ng (or espeak / speech-dispatcher) for voice output.")
                return
        ...
        try:
            for char in text:
                if char.isupper() or char in '!@#$%^&*()_+{}|:"<>?~':
                    try:
                        client.keyDown(SCANCODES["shift"])
                        key_to_send = char.lower() if char.isupper() else char
                        client.keyPress(key_to_send)
                    finally:
                        try:
                            client.keyUp(SCANCODES["shift"])
                        except Exception:
                            pass
                else:
...
def handle_mouse(cmd, args):
    ...
    with _vnc_lock:
        ...
        try:
            ...
            def _move_to(x, y):
                _vnc_cursor["x"] = max(0, min(1920, x))
                _vnc_cursor["y"] = max(0, min(1080, y))
                client.mouseMove(_vnc_cursor["x"], _vnc_cursor["y"])
            ...
            if cmd in ['move', 'mouse', 'mv', 'm']:
                ...
                if len(parts) >= 2:
                    ...
                    _move_to(_vnc_cursor["x"] + dx, _vnc_cursor["y"] + dy)
                    client.mouseDrag(_vnc_cursor["x"], _vnc_cursor["y"])
    ...
    while not bot_stop_event.is_set():
        ...
        if restart_start_time is not None:
            ...
            if elapsed > VOTE_TIMEOUT:
                ...
                update_votes_json("restartvm", 0, get_vote_threshold("restart_votes", 2), 0)
        ...
        if revert_start_time is not None:
            ...
            if elapsed > VOTE_TIMEOUT:
                ...
                update_votes_json("revert", 0, get_vote_threshold("revert_votes", 2), 0)
...
def beep():
    ...
    if platform.system() == "Darwin":
        ...
        import shutil as _shutil
        if _shutil.which("canberra-gtk-play"):
            try:
                subprocess.run(["canberra-gtk-play", "--id", "bell"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5)
                return
            except Exception:
                pass
...
def vm_shutdown_soft():
    ok, err = retry_vbox(lambda: _checked(vm_stop(VMX_PATH, current_vm_backend, hard=False)),
                          source="Cmd/shutdown")
    if ok:
...
def vm_shutdown_hard_kill():
    ok, err = retry_vbox(lambda: _checked(vm_stop(VMX_PATH, current_vm_backend, hard=True)),
                          source="Cmd/killvm")
    if ok:
        obs_trigger("vm_shutdown")
    return ok, err
...
def vm_make_snapshot(name):
    ...
    return retry_vbox(lambda: _checked(vm_take_snapshot(VMX_PATH, name, current_vm_backend)),
...
def vm_set_internet_live(enabled, os_type=None, iface=None):
    ...
    one). VBox: uses VBoxManage's per-VM setlinkstate1 instead, which only affects this
    specific VM and needs no host service."""
    if current_vm_backend == "vbox":
        try:
            ...
            return False, str(e)
    if platform.system() == "Darwin":
        # VMware Fusion has no Windows-style service -- NAT networking runs under a
        # LaunchDaemon (com.vmware.launchd.vmnet). Needs admin privileges; osascript's
        # "with administrator privileges" pops the native macOS auth dialog since raw
        # sudo doesn't work from a GUI app. HONEST NOTE: the exact LaunchDaemon label can
        # vary by Fusion version -- if this fails, check `sudo launchctl list | grep
        # vmware` on your machine and adjust to match.
        try:
            ...
            INTERNET_CONFIG["enabled"] = enabled
            save_internet_config()
            return result.returncode == 0, output.strip() or f"launchctl {verb} exited {result.returncode}"
        except Exception as e:
            return False, str(e)
    elif platform.system() == "Linux":
        # VMware Workstation on Linux manages NAT networking through a systemd service
        # (usually vmware-networking). pkexec is the modern Linux equivalent of osascript's
        # "with administrator privileges" -- pops a native polkit auth dialog instead of
        # needing a raw sudo password prompt a GUI app has no way to display.
        # HONEST NOTE: the exact service name can vary by distro/Workstation version -- if
        # this fails, check `systemctl list-units | grep vmware` on your machine and adjust
        # to match, or try `sudo vmware-networks --start`/`--stop` (the bundled CLI tool) as
        # an alternative if the systemd unit name genuinely differs.
        try:
            verb = "start" if enabled else "stop"
            result = subprocess.run(["pkexec", "systemctl", verb, "vmware-networking"],
                                     capture_output=True, text=True, timeout=30)
            output = (result.stdout or "") + (result.stderr or "")
            INTERNET_CONFIG["enabled"] = enabled
            save_internet_config()
            return result.returncode == 0, output.strip() or f"systemctl {verb} exited {result.returncode}"
        except Exception as e:
            return False, str(e)
    try:
        args = ["net", "start" if enabled else "stop", "VMware NAT Service"]
...
def run_admin_cmd(command_text):
    """!cmd -- opens an admin Command Prompt inside the VM (via Win+R) and runs `command_text`.
    Elevation depends on the VM's own UAC settings; VNC can't force elevation itself."""
...
def mouse_circle(radius=100, steps=24, loops=1):
    def _run():
        cx, cy = _vnc_cursor["x"], _vnc_cursor["y"]
...
def cmd_status_text():
    ...
    vm_state = "running" if vm_is_running(VMX_PATH, current_vm_backend) else "not running"
...
class UltraBotGUI:
    ...
    def __init__(self, root):
        ...
        # Applies to both the GUI widgets AND the global current_vm_backend/VMX_PATH
    ...
    def _build_ui(self):
        ...
        # The ttk.Notebook tab strip doesn't scroll natively, and there's no reliable
        # way to hook a real horizontal scrollbar up to it (ttk.Notebook doesn't expose
        # an internal "xview" the way Text/Canvas do -- an earlier attempt at this
        # ended up as a scrollbar that was always visible but did nothing when
        # dragged, which is worse than not having one at all). Mousewheel-to-cycle and
        # Ctrl-Tab/Ctrl-Shift-Tab below are the actual, working way to reach a tab that
        # doesn't fit in the visible strip.
        ...
        tab4  = ttk.Frame(nb)   # OS Voting already has its own internal Canvas+Scrollbar
        tab5_outer,  tab5  = self._make_scrollable_tab(nb)
        tab6  = ttk.Frame(nb)   # OBS already has its own internal Canvas+Scrollbar
        ...
        tab14 = ttk.Frame(nb)   # Real PC already has its own internal Canvas+Scrollbar
    ...
    def _vm_restart(self):
        if not VMX_PATH:
            messagebox.showerror("No VM", "Start the bot first to select a VM.")
            return
        if not messagebox.askyesno("Restart VM", f"Reset '{VMX_PATH}' now?"):
            return
    ...
    def _vm_revert(self):
        ...
        if not messagebox.askyesno("Revert VM",
                f"Power off '{VMX_PATH}', restore snapshot and reboot?\n"
    ...
    def _add_vm_entry(self):
        ...
        vmx   = self._reg_vmx_var.get().strip().strip('"')
        if not alias or not vmx:
            messagebox.showwarning("Missing", "Enter both an alias and a .vmx path.")
            return
        if not vmx.lower().endswith(".vmx"):
            messagebox.showwarning("Invalid Path", "The path should point to a .vmx file.")
            return
        registry = _load_vm_registry()
        registry[alias] = vmx
        _save_vm_registry(registry)
        self._reg_alias_var.set("")
        self._reg_vmx_var.set("")
        self._refresh_vm_list()
        if hasattr(self, "_refresh_os_vm_lists"):
            self._refresh_os_vm_lists()
        self._log(f"[VM] Registered: {alias} -> {vmx}")

    # ──────────────── Bot Start / Stop ────────────────
    def _on_test_mode_toggle(self):
        global TEST_MODE_ENABLED, VMX_PATH, current_os_vm, current_vm_backend
        ...
        if enabled:
            ...
            if OS_VOTING_ENABLED:
                ...
                if valid:
                    VMX_PATH = valid[0]["vm"]
                    current_os_vm = VMX_PATH
                    current_vm_backend = valid[0].get("backend", "vmware")
            else:
                VMX_PATH = vm
                current_os_vm = vm
            ...
            self._log(f"[TestMode] Started. VM: {VMX_PATH} (backend: {current_vm_backend}). Type commands in the console.")
            notify("Test Mode Active", f"VM: {VMX_PATH}\nType commands in the console window.")
            save_autostart_everything_config(backend=current_vm_backend, vm=VMX_PATH, test_mode=True)
        else:
            bot_stop_event.set()
    ...
    def _build_sound_tts_tab(self, parent):
        ...
        # Test TTS
        self._tts_test_var = tk.StringVar(value="VMware Chat Bot is ready!")
    ...
    # ==================== VMWARE VNC + WEB DASHBOARD TAB ====================
    def _build_vnc_web_tab(self, parent):
        parent.configure(style="TFrame")

        hdr = tk.Frame(parent, bg=self.BG)
        hdr.pack(fill="x", padx=16, pady=(14, 6))
        tk.Label(hdr, text="🖧  VMware VNC + Web Dashboard",
                 bg=self.BG, fg=self.ACCENT,
                 font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(hdr,
                 text="Set the VNC console the bot injects keyboard/mouse input into, "
                      "and run the multistream web dashboard (chat overlays mixed from both scripts).",
                 bg=self.BG, fg=self.TEXTDIM, font=("Segoe UI", 9)).pack(anchor="w", pady=(2, 0))

        # ── VNC card ──
        vnc_card = ttk.Frame(parent, style="Card.TFrame", padding=14)
        vnc_card.pack(fill="x", padx=12, pady=(8, 8))
        tk.Label(vnc_card, text="VMware VNC Connection", bg=self.BG2, fg=self.TEXT,
                 font=("Segoe UI", 10, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 8))

        tk.Label(vnc_card, text="Host:", bg=self.BG2, fg=self.TEXTDIM,
                  font=("Segoe UI", 9)).grid(row=1, column=0, sticky="w", pady=4)
        self._vnc_host_var = tk.StringVar(value=VNC_CONFIG.get("host", "localhost"))
        ttk.Entry(vnc_card, textvariable=self._vnc_host_var, width=24,
                   font=("Segoe UI Mono", 10)).grid(row=1, column=1, sticky="w", padx=(8, 0), ipady=3)

        tk.Label(vnc_card, text="Port:", bg=self.BG2, fg=self.TEXTDIM,
                  font=("Segoe UI", 9)).grid(row=2, column=0, sticky="w", pady=4)
        self._vnc_port_var = tk.StringVar(value=str(VNC_CONFIG.get("port", 5900)))
        ttk.Entry(vnc_card, textvariable=self._vnc_port_var, width=10,
                   font=("Segoe UI Mono", 10)).grid(row=2, column=1, sticky="w", padx=(8, 0), ipady=3)

        tk.Label(vnc_card, text="Password:", bg=self.BG2, fg=self.TEXTDIM,
                  font=("Segoe UI", 9)).grid(row=3, column=0, sticky="w", pady=4)
        self._vnc_pass_var = tk.StringVar(value=VNC_CONFIG.get("password", ""))
        ttk.Entry(vnc_card, textvariable=self._vnc_pass_var, width=24, show="•",
                   font=("Segoe UI Mono", 10)).grid(row=3, column=1, sticky="w", padx=(8, 0), ipady=3)

        vnc_btn_row = tk.Frame(vnc_card, bg=self.BG2)
        vnc_btn_row.grid(row=4, column=0, columnspan=2, sticky="w", pady=(10, 0))
        ttk.Button(vnc_btn_row, text="💾 Save", style="Green.TButton",
                   command=self._vnc_save_config).pack(side="left", padx=(0, 8))
        ttk.Button(vnc_btn_row, text="🔌 Test / Reconnect", style="Dim.TButton",
                   command=self._vnc_test_connect).pack(side="left")

        self._vnc_status_lbl = tk.Label(vnc_card, text="", bg=self.BG2, fg=self.TEXTDIM,
                                        font=("Segoe UI", 8, "italic"))
        self._vnc_status_lbl.grid(row=5, column=0, columnspan=2, sticky="w", pady=(6, 0))

        # ── Web / Multistream dashboard card ──
        web_card = ttk.Frame(parent, style="Card.TFrame", padding=14)
        web_card.pack(fill="x", padx=12, pady=(0, 8))
        ...
        tk.Label(web_card,
                 text="Serves every OBS/browser overlay page (mixed in from both scripts) on one port.",
        ...
        ttk.Button(web_btn_row, text="🌐 Open", style="Dim.TButton",
    ...
    def _vnc_save_config(self):
        global VNC_HOST, VNC_PORT, VNC_PASSWORD
        host = self._vnc_host_var.get().strip() or "localhost"
        try:
            port = int(self._vnc_port_var.get().strip())
        except Exception:
            messagebox.showerror("VMware VNC", "Port must be a number.")
            return
        password = self._vnc_pass_var.get()
        VNC_CONFIG["host"] = host
        VNC_CONFIG["port"] = port
        VNC_CONFIG["password"] = password
        save_vnc_config()
        VNC_HOST, VNC_PORT, VNC_PASSWORD = host, port, password
        self._vnc_status_lbl.configure(text=f"Saved. {host}::{port}. Reconnect to apply.")
        self._log(f"[VNC] Config saved: {host}::{port}")

    def _vnc_test_connect(self):
        self._vnc_save_config()
        def _try():
            try:
                vnc_connect_fresh()
                self._vnc_status_lbl.configure(text=f"Connected to {VNC_HOST}::{VNC_PORT}.")
                self._log(f"[VNC] Connected to {VNC_HOST}::{VNC_PORT}")
            except Exception as e:
                self._vnc_status_lbl.configure(text=f"Connection failed: {e}")
                self._log(f'[VNC] Connection failed: Python exception: "{traceback.format_exc()}"')
    ...
    def show_welcome_guide(self, force=False):
        ...
        CHAPTERS = [
            ...
            ("🌐  Web Dashboard", [
                ...
                ("h2",   "Starting it"),
                ("body", "Go to the  🖧 VNC / Web  tab, pick a port between 5900-5999, and click  ▶ Start."),
            ...
            ("🖧  VMware VNC", [
                ("h1",   "🖧  VMware VNC"),
                ("body", "All keyboard/mouse input to the VM is delivered over VNC, since vmrun itself has no keystroke/mouse injection capability. This tab lets you point that VNC connection at the right VM."),
                ("h2",   "Setup"),
                ("body", "1.  Enable the VM's VNC server in its .vmx file, e.g.:"),
                ("code", "  RemoteDisplay.vnc.enabled  = \"TRUE\""),
                ("code", "  RemoteDisplay.vnc.port     = \"5900\""),
                ("code", "  RemoteDisplay.vnc.password = \"1234\""),
                ("body", "2.  Go to the  🖧 VNC / Web  tab and enter the matching Host, Port, and Password."),
                ("body", "3.  Click  💾 Save,  then  🔌 Test / Reconnect  to verify the connection."),
                ("h2",   "How it behaves"),
                ("body", "Every key press, combo, and mouse action opens a brand-new VNC connection, sends that one action, then disconnects immediately -- it is never held open between commands."),
                ("tip",  "Tip: if you have more than one VM registered, double-check the port/password actually corresponds to the VM you intend to control -- VNC will happily connect to whichever VM answers on that port."),
                ("tip",  "Tip: settings are saved to  vmware_vnc_config.json."),
            ]),
            ("🌍  VM Internet", [
                ("h1",   "🌍  VM Internet"),
                ...
                ("tip",  "Tip: requires the bot to be running with administrator rights, since controlling a Windows service needs elevation -- already covered by this app's UAC self-elevation on launch."),
            ]),
        ]
