...
#   Browsable page: https://github.com/mrtristin449/TheUltimateYouTubeChatUsesVMsPythonScriptLiveBot247ScriptMrTristin/blob/main/YouTubeChatUsesVMs/YouTubeChatUsesVM-Windows-VMware.py
...
AUTOUPDATE_SCRIPT_URL  = "https://raw.githubusercontent.com/mrtristin449/TheUltimateYouTubeChatUsesVMsPythonScriptLiveBot247ScriptMrTristin/main/YouTubeChatUsesVMs/YouTubeChatUsesVM-Windows-VMware.py"
...
def _save_all_configs_before_relaunch():
    ...
    was killed (os._exit() + taskkill give zero opportunity for cleanup code to run).
...
def vmrun_is_running(vmx_path: str) -> bool:
    ...
    if not vmx_path or not VMRUN_PATH or not os.path.exists(VMRUN_PATH):
        return False
    try:
        result = subprocess.run([VMRUN_PATH, "-T", "ws", "list"],
...
def vm_start(target, backend=None, gui=True):
    ...
    return subprocess.run([VMRUN_PATH, "-T", "ws", "start", target, "gui" if gui else "nogui"], capture_output=True, text=True, timeout=30)

def vm_stop(target, backend=None, hard=True):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_stop(target, hard, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", "ws", "stop", target, "hard" if hard else "soft"], capture_output=True, text=True, timeout=20)

def vm_reset(target, backend=None):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_reset(target, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", "ws", "reset", target, "hard"], capture_output=True, text=True, timeout=20)
...
def vm_revert_to_snapshot(target, snapshot_name, backend=None):
    ...
    return subprocess.run([VMRUN_PATH, "-T", "ws", "revertToSnapshot", target, snapshot_name], capture_output=True, text=True, timeout=60)

def vm_take_snapshot(target, snapshot_name, backend=None):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_take_snapshot(target, snapshot_name, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", "ws", "snapshot", target, snapshot_name], capture_output=True, text=True, timeout=60)
...
def _kill_backend_processes(vm_name, backend, nuclear=False):
    """Force-kills the OS-level process(es) actually backing a specific VM. Windows has
    no direct equivalent to pkill -f (matching a specific process by command-line
    substring, not just image name) -- taskkill /IM alone would kill EVERY VMware/VBox
    VM on the machine, not just this one, so this uses PowerShell's Get-CimInstance to
    filter by command line containing this VM's own path first, then kills only that
    specific PID. Used as an escalation step when the backend's own stop/poweroff
    command hasn't actually gotten the VM to stop within the allotted time.
    nuclear=True additionally widens the match, for the final, most aggressive tier."""
    try:
        escaped = vm_name.replace("'", "''")   # PowerShell single-quote escaping
        if backend == "vmware":
            image_names = "'vmware-vmx.exe'" if not nuclear else "'vmware-vmx.exe','vmware.exe'"
        elif backend == "vbox":
            image_names = "'VBoxHeadless.exe','VBoxSDL.exe'" if not nuclear else "'VBoxHeadless.exe','VBoxSDL.exe','VirtualBoxVM.exe'"
        else:
            return
        ps_cmd = (
            f"Get-CimInstance Win32_Process | "
            f"Where-Object {{ $_.Name -in @({image_names}) -and $_.CommandLine -like '*{escaped}*' }} | "
...
def vm_pause():
    if current_vm_backend == "vbox":
        return retry_vbox(lambda: _checked(subprocess.run([VBOXMANAGE_PATH, "controlvm", VMX_PATH, "pause"], capture_output=True, text=True)),
                           source="Cmd/pausevm")
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'pause', VMX_PATH], check=True, timeout=20),
                       source="Cmd/pausevm")
...
def vm_unpause():
    if current_vm_backend == "vbox":
        return retry_vbox(lambda: _checked(subprocess.run([VBOXMANAGE_PATH, "controlvm", VMX_PATH, "resume"], capture_output=True, text=True)),
                           source="Cmd/resumevm")
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'unpause', VMX_PATH], check=True, timeout=20),
                       source="Cmd/resumevm")
...
def vm_save_state():
    ...
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'suspend', VMX_PATH], check=True, timeout=30),
                       source="Cmd/vmsavestate")
