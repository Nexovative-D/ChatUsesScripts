    ...
    if platform.system() == "Windows":
        ...
        try:
            if _shutil.which("zenity"):
                # zenity's own exit code IS the answer -- 0 for Yes, anything else for No.
            ...
            return True
        # Neither zenity nor kdialog found (e.g. a headless/server box without a desktop
        # environment) -- fall back to a console prompt rather than silently blocking.
        try:
...
CURRENT_FILENAME = "YouTubeChatUsesVM-Linux-VBox.py"

# Replace these two URLs with your own GitHub repo paths.
# GITHUB_VERSION_URL  → raw URL of version.json in your repo
# GITHUB_SCRIPT_URL   → raw URL of THIS file (YouTubeChatUsesVM-Linux-VBox.py) in your repo
#   Browsable page: https://github.com/mrtristin449/TheUltimateYouTubeChatUsesVMsPythonScriptLiveBot247ScriptMrTristin/blob/main/YouTubeChatUsesVMs/YouTubeChatUsesVM-Linux-VBox.py
...
AUTOUPDATE_SCRIPT_URL  = "https://raw.githubusercontent.com/mrtristin449/TheUltimateYouTubeChatUsesVMsPythonScriptLiveBot247ScriptMrTristin/main/YouTubeChatUsesVMs/YouTubeChatUsesVM-Linux-VBox.py"
...
def vmware_toggle_internet(enable, vmrun_path=None):
    ...
    if platform.system() == "Darwin":
        ...
        verb = "start" if enable else "stop"
        try:
            return subprocess.run(["pkexec", "systemctl", verb, "vmware-networking"],
